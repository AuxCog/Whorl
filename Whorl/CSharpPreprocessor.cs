...
";
        ...
        private bool ProcessParameterDec(ref int tokenIndex, Token directiveToken, Dictionary<string, BaseParamInfo> parametersDict)
        {
            ...
            {
                nameTok = GetToken(tokenIndex++);
                if (!ParseReservedWord(nameTok.Text, ReservedWords.NestedParameters))
                {
            ...
            if (isNestedParamsParam)
            {
                ...
                {
                    tokenIndex++;
                    typeName = ParseType(ref tokenIndex, "Expecting type of nested parameter class.");
                    if (typeName != null)
                        isValid = true;
                }
                if (!isValid)
            ...
            else
            {
                if (tok.TokenType == Token.TokenTypes.LeftBracket)
                {
                    ...
                    else if (tok.TokenType == Token.TokenTypes.Number)
                    {
        ...
        private bool? TryParseType(ref int tokenIndex, out List<Token> tokens)
        {
            ...
            bool? isType = TryParseType(tokens, ref tokInd);
            if (isType != false)
                tokenIndex = tokInd;
            return isType;
        }
        ...
        private bool? TryParseType(List<Token> tokens, ref int tokInd)
        {
            ...
            while (true)
            {
                ...
                else if (tok.Text == "<")
                {
                    ...
                    while (true)
                    {
                        if (TryParseType(tokens, ref tokInd) == false)
                        {
                            isType = false;
                            break;
                        }
        ...
        {
            string paramsClassName = null, defaultValue = null;
            Token token = GetToken(tokenIndex);
        ...
        {
            var arrayParams = GetParamInfos<ParamInfo>(ParamsDict.Values).Where(pi => pi.IsArray && !pi.IsNonParameter);
        ...
        {
            return ParseReservedWord(text, out _, validWord);
        }
